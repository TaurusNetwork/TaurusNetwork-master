...
# Set these to change the version of TaurusNetwork to install
TARBALLURL="https://github.com/taurusnetworkGroup/TaurusNetwork/releases/download/v1/TaurusNetwork-lin-bin.tar.gz"
TARBALLNAME="TaurusNetwork-lin-bin.tar.gz"
FGCVERSION="1.0"
...
echo "" && echo 'Added user "taurusnetwork"' && echo ""
sleep 1
...
# Install TaurusNetwork daemon
wget $TARBALLURL
tar -xzvf $TARBALLNAME #&& mv bin taurusnetwork-$FGCVERSION
rm $TARBALLNAME
cp ./taurusnetworkd /usr/local/bin
cp ./taurusnetwork-cli /usr/local/bin
cp ./taurusnetwork-tx /usr/local/bin
rm -rf taurusnetwork-$FGCVERSION

# Create .taurusnetwork directory
mkdir $USERHOME/.taurusnetwork

# Install bootstrap file
#if [[ ("$BOOTSTRAP" == "y" || "$BOOTSTRAP" == "Y" || "$BOOTSTRAP" == "") ]]; then
#  echo "Installing bootstrap file..."
#  wget $BOOTSTRAPURL && unzip $BOOTSTRAPARCHIVE -d $USERHOME/.taurusnetwork/ && rm $BOOTSTRAPARCHIVE
#fi

# Create taurusnetwork.conf
touch $USERHOME/.taurusnetwork/taurusnetwork.conf
cat > $USERHOME/.taurusnetwork/taurusnetwork.conf << EOL
rpcuser=${RPCUSER}
...
masternode=1
EOL
chmod 0600 $USERHOME/.taurusnetwork/taurusnetwork.conf
chown -R $USER:$USER $USERHOME/.taurusnetwork
...
ExecStart=/usr/local/bin/taurusnetworkd -conf=${USERHOME}/.taurusnetwork/taurusnetwork.conf -datadir=${USERHOME}/.taurusnetwork
ExecStop=/usr/local/bin/taurusnetwork-cli -conf=${USERHOME}/.taurusnetwork/taurusnetwork.conf -datadir=${USERHOME}/.taurusnetwork stop
Restart=on-abort
[Install]
...
sleep 1
clear
su -c "/usr/local/bin/taurusnetwork-cli masternode status" $USER
sleep 5
