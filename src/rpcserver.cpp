...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop TaurusNetwork server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "TaurusNetwork server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* TAN features */
        {"taurusnetwork", "masternode", &masternode, true, true, false},
        {"taurusnetwork", "masternodelist", &masternodelist, true, true, false},
        {"taurusnetwork", "mnbudget", &mnbudget, true, true, false},
        {"taurusnetwork", "mnbudgetvoteraw", &mnbudgetvoteraw, true, true, false},
        {"taurusnetwork", "mnfinalbudget", &mnfinalbudget, true, true, false},
        {"taurusnetwork", "mnsync", &mnsync, true, true, false},
        {"taurusnetwork", "spork", &spork, true, true, false},
#ifdef ENABLE_WALLET
        {"taurusnetwork", "Darksend", &Darksend, false, false, true}, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
    ...
    if (((mapArgs["-rpcpassword"] == "") ||
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
                                             _("To use taurusnetworkd, or the -server option to taurusnetwork-qt, you must set an rpcpassword in the configuration file:\n"
                                               "%s\n"
                                               ...
                                               "for example: alertnotify=echo %%s | mail -s \"TaurusNetwork Alert\" admin@foo.com\n"),
...
{
    return "> taurusnetwork-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(string methodname, string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:18403/\n";
}
