...
#if defined(NDEBUG)
#error "TaurusNetwork cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "TaurusNetwork Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 1) return 7074396 * COIN;

	int64_t nSubsidy;

	if( nHeight > 1 && nHeight <= 50000 ) {
	        nSubsidy = 200 * COIN;
...
{
    RenameThread("taurusnetwork-scriptch");
