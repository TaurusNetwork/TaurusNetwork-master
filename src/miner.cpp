...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TANMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("TANMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("TANMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("taurusnetwork-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("TANMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running TANMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadTANMiner() exception");
    } catch (...) {
        LogPrintf("ThreadTANMiner() exception");
    }

    LogPrintf("ThreadTANMiner exiting\n");
}
